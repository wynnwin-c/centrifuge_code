...
TRANSFER_MODELS = {"ddc", "dann", "edann", "ladv", "sladv", "san", "bsan", "bsdan"}
PARTIAL_TRANSFER_MAP = {"ddc": "mmd", "dann": "adv", "edann": "edann", "ladv": "ladv", "sladv": "sladv", "san": "san", "bsan": "bsan", "bsdan": "bsdan"}
...
def split_target_by_group(dataset: NpyDataset, seed: int, test_ratio: float = 0.34, use_full_target: bool = False):
    if use_full_target:
        ...
        for cls in [0, 1]:
            ...
            split_info[int(cls)] = {
                'train_groups': cls_groups.tolist(),
                'test_groups': cls_groups.tolist(),
                'train_samples': int(len(cls_idx)),
                'test_samples': int(len(cls_idx)),
            }
        subset = Subset(dataset, full_indices)
        return subset, subset, split_info
...
def main():
    ...
    parser.add_argument("--model", type=str, choices=["source_only", "ddc", "dann", "edann", "pdan", "ladv", "sladv", "san", "bsan", "bsdan"], default="bsan")
    ...
    use_full_target_split = bool(cfg["train"].get("use_full_target_split", True))
    ...
    ds_tgt_train, ds_tgt_test, split_info = split_target_by_group(ds_tgt_full, args.seed, target_group_test_ratio, use_full_target_split)
    ...
    if args.model == "source_only":
        ...
        transfer_loss_name = PARTIAL_TRANSFER_MAP[args.model]
        use_partial_plain = args.model in {'ddc', 'dann', 'edann', 'san'}
        model = TransferNet(num_class=num_classes, model_name='cnn_features', in_channel=in_channels, transfer_loss=transfer_loss_name, max_iter=min(len(source_train), len(target_train)) * epochs, trade_off_adversarial='Step', lam_adversarial=1, batch_size=batch_size, shared_classes=task_cfg.get('shared_classes', []), aux_source_weight=0.0 if use_partial_plain else aux_source_weight, aux_target_weight=0.0 if use_partial_plain else aux_target_weight, pseudo_threshold=0.0 if use_partial_plain else pseudo_threshold, pseudo_topk=0 if use_partial_plain else pseudo_topk, pseudo_warmup_epochs=0 if use_partial_plain else pseudo_warmup_epochs)
    else:
